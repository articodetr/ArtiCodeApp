/*
  Enforce approval-before-posting for linked "له / عليه" movements.

  Core behavior:
  - Any linked-counterparty movement starts as pending.
  - Pending / rejected movements never affect balances or totals.
  - Notes are required on movement creation.
  - Approval / rejection keeps audit fields on the movement rows.
*/
...
BEGIN
  ...
  PERFORM create_notification(
    ...
    'approval_needed',
    format(
      'لديك حركة %s بانتظار الموافقة من %s بمبلغ %s %s',
      CASE
        WHEN v_original_movement.movement_type = 'incoming' THEN 'له'
        ELSE 'عليه'
      END,
      v_actor_name,
...
BEGIN
  ...
  IF v_creator_user_id IS NOT NULL AND v_creator_user_id <> v_user_id THEN
    PERFORM create_notification(
      ...
      v_creator_user_id,
      'movement_approved',
      format('تم اعتماد الحركة من %s', p_user_name),
