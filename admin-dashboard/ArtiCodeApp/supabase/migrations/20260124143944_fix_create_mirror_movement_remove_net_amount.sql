/*
  # Fix create_mirror_movement_v2 - Remove net_amount Column References

  ## Problem
  The `create_mirror_movement_v2` function was referencing a `net_amount` column that doesn't exist 
  in the `account_movements` table. This column was removed in a previous migration when the old 
  commission system was removed.

  ## Changes
  - Recreate `create_mirror_movement_v2` function without `net_amount` references
  - Keep all other fields: amount, commission, commission_currency, commission_recipient
  - Use `notes` field instead of `description` for consistency with the schema

  ## Impact
  - Fixes error: column "net_amount" of relation "account_movements" does not exist
  - Allows mirror movements to be created successfully for linked accounts
*/

-- Drop and recreate the function without net_amount
DROP FUNCTION IF EXISTS create_mirror_movement_v2(uuid);
...
BEGIN
  ...
  -- إنشاء الحركة المرآة (بدون net_amount)
...
COMMENT ON FUNCTION create_mirror_movement_v2 IS 'إنشاء حركة مرآة للحسابات المرتبطة - تم إصلاحه بإزالة net_amount';
