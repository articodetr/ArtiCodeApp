/*
  # منع العمولة عند التحويل من المحل إلى العميل
  
  ## التغييرات
  
  1. تحديث دالة `create_internal_transfer`:
     - إضافة تحقق يرفض العمولة إذا كان المرسل هو المحل (p_from_customer_id IS NULL)
     - إذا تم تمرير عمولة في هذه الحالة، يتم تجاهلها تلقائياً
  
  ## السبب
  
  - التحويلات من المحل إلى العملاء لا يجب أن تحتوي على عمولة
  - هذا يمنع أي محاولة لإضافة عمولة من الواجهة الأمامية أو API مباشرة
*/
...
COMMENT ON FUNCTION create_internal_transfer IS 'إنشاء تحويل داخلي - العمولة محظورة عند التحويل من المحل إلى العميل';