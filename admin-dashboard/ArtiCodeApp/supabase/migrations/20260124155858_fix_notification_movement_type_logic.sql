/*
  # إصلاح منطق نوع الحركة في الإشعارات

  1. Changes
    - تصحيح movement_type في الإشعارات ليعكس نوع الحركة الأصلية
    - إذا كانت الحركة الأصلية "outgoing" → الإشعار يعرض "outgoing"
    - إذا كانت الحركة الأصلية "incoming" → الإشعار يعرض "incoming"
    - هذا يضمن أن المستقبل يرى نفس المعنى (عليه/له) كما سجله المنشئ

  2. Logic
    - المنشئ: يسجل "عميل عليه" (outgoing)
    - المستقبل: يرى "عليك" (outgoing) في الإشعار
*/

-- إعادة إنشاء الدالة مع المنطق الصحيح
...
BEGIN
  ...
  -- ✅ نرسل نوع الحركة الأصلية في الإشعار (وليس المرآة)
  IF v_mirror_needs_approval THEN
    PERFORM create_notification(
      ...
      v_customer.name,
      v_customer.name,
      v_original_movement.movement_type,  -- ✅ نوع الحركة الأصلية (وليس المرآة)
      NULL
    );
  END IF;
...
COMMENT ON FUNCTION create_mirror_movement_v2 IS 'إنشاء حركة مرآة مع منطق صحيح لنوع الحركة في الإشعار';
