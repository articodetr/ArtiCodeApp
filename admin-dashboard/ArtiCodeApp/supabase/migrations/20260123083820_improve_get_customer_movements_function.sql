/*
  # تحسين دالة جلب حركات العميل لتشمل معلومات العميل المرتبط

  ## التحسينات
  - إضافة معلومات العميل المرتبط (linked_user) لكل حركة
  - إرجاع البيانات بصيغة JSON تشمل كل التفاصيل المطلوبة
*/

-- حذف الدالة القديمة
DROP FUNCTION IF EXISTS get_customer_movements_with_user(text, uuid);
...
BEGIN
  ...
  -- جلب الحركات مع معلومات العميل المرتبط
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', am.id,
