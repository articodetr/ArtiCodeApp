...
AS $$
DECLARE
  v_seen uuid[] := ARRAY[p_movement_id]::uuid[];
  v_next uuid[];
...
BEGIN
  ...
  /*
    This function collects all rows that belong to the same logical movement:
    - the original movement
    - mirror movement
    - related transfer movement
    - related commission movement
    - any rows pointing back to one of them

    It uses a safe PL/pgSQL loop instead of a recursive CTE because PostgreSQL
    does not allow recursive self-reference in a non-recursive term.
  */
  ...
  LOOP
    ...
    INTO v_next
    FROM (
      SELECT am.mirror_movement_id AS candidate_id
      FROM public.account_movements am
      WHERE am.id = ANY(v_seen)
        AND am.mirror_movement_id IS NOT NULL

      UNION

      SELECT am.related_transfer_id AS candidate_id
      FROM public.account_movements am
      WHERE am.id = ANY(v_seen)
        AND am.related_transfer_id IS NOT NULL

      UNION

      SELECT am.related_commission_movement_id AS candidate_id
      FROM public.account_movements am
      WHERE am.id = ANY(v_seen)
        AND am.related_commission_movement_id IS NOT NULL

      UNION

      SELECT am.id AS candidate_id
      FROM public.account_movements am
      WHERE am.id IS NOT NULL
        AND (
          am.mirror_movement_id = ANY(v_seen)
          OR am.related_transfer_id = ANY(v_seen)
          OR am.related_commission_movement_id = ANY(v_seen)
        )
    ) candidates
    WHERE candidate_id IS NOT NULL
      AND NOT (candidate_id = ANY(v_seen));

    EXIT WHEN COALESCE(array_length(v_next, 1), 0) = 0;

    SELECT array_agg(DISTINCT id)
    INTO v_seen
    FROM unnest(v_seen || v_next) AS id;

    -- Safety guard to avoid infinite loops if old data has circular links.
    EXIT WHEN v_loop_guard >= 10;
  END LOOP;

  RETURN COALESCE(v_seen, ARRAY[p_movement_id]::uuid[]);
END;
$$;
