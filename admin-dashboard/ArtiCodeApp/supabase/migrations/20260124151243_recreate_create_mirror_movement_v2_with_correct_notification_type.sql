/*
  # إعادة إنشاء دالة create_mirror_movement_v2 بنوع إشعار صحيح
  
  ## التغيير
  إعادة إنشاء الدالة بعد حذفها لضمان استخدام 'approval_needed' بدلاً من 'pending_approval'
*/
...
BEGIN
  ...
  IF v_mirror_needs_approval THEN
    PERFORM create_notification(
      ...
      v_customer.linked_user_id,
      'approval_needed',  -- ✅ الآن يستخدم النوع الصحيح
...
COMMENT ON FUNCTION create_mirror_movement_v2 IS 'إنشاء حركة مرآة للحسابات المرتبطة مع نوع إشعار صحيح (approval_needed)';

-- إعادة إنشاء الـ triggers التي تستخدم هذه الدالة
...
-- Trigger: إنشاء حركة مرآة عند INSERT
...
-- Trigger: إنشاء حركة مرآة عند الموافقة
